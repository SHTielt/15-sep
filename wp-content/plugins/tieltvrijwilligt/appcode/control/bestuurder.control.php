...
if(isset($_POST['btnBestuurderSave']))
{
		...
		*/
        ...
        if($result)
        {
            header('Location: http://localhost:8080/sociaalhuis/organisatie-formulier-bestuur?organisatieid='.$verId);
        }
...
if (isset($_GET['actie']) && isset($_GET['bestuurderid']))
{
    ...
    if(!empty($result))
    {
        header('Location: http://localhost:8080/sociaalhuis/organisatie-formulier?organisatieid=');
    }
    else
    {
    	$message = $interesseObject->getFeedback();
