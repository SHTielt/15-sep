...
if(isset($_POST['btnContactPersoonSave']))
{
		...
		//{
        	...
        	if($result)
        	{
            	header('Location: http://localhost:8080/sociaalhuis/organisatie-formulier-contactpersoon?organisatieid='.$verId);
        	}
...
//delete
if (isset($_GET['contactpersoonid']))
{
    echo "contactpersoonid: ".$_GET['contactpersoonid'];
    $contObject = new ContactPersoon();
    $contactPersoonId = $_GET['contactpersoonid'];
	
	//verId ophalen nodig voor URL
	$gezochteContactPersoon = $contObject->selectContactPersoonById($contactPersoonId);
	$verId = $gezochteContactPersoon[0]['verID'];
    $result = $contObject->delete($contactPersoonId);
    if($result)
    {
         header('Location: http://localhost:8080/sociaalhuis/organisatie-formulier-contact?organisatieid='.$verId);
    }
    else
    {
    	//$message = $interesseObject->getFeedback();
        $message = "De contactpersoon is niet verwijderd. Probeer later opnieuw of contacteer de administrator.";
        $_SESSION['message'] = $message;
        echo $message;
